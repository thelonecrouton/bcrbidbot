...
class CC(QtGui.QWidget):
    ...
    def initUI(self):
        global t1, tcbval, estval, cmvval, until, c2, c4, t2, b1, ed, b3, e, c3b, status, cmvval, edv, f
	...
	####################################################################################################
        ...
        QLabel#logo {background-image: url('head.png');}
        """) 
    ...
    def scheduledbid(self):
        global status, c2
        status.setText('Status: scheduled bid time has arrived, making bid...')
        try:
            # create tx
    ...
    def callbidthread(self):
        ...
        if b1.isChecked():
            ...
            hours = hhmmss.split(':')[0]
            mins = hhmmss.split(':')[1]
            secs = hhmmss.split(':')[2]
            current = dt.today()
            sched = current.replace(day=current.day+1, hour=int(hours), minute=int(mins), second=int(secs), microsecond=0)
            #sched = current.replace(day=current.day, hour=int(hours), minute=int(mins), second=int(secs), microsecond=0)
            delta_t = sched - current
            seconds = delta_t.seconds+1
            t = Timer(seconds, self.scheduledbid)
            t.start()
            status.setText('Status: Bid scheduled for...' + hhmmss)
